...
export class PlaybackEngine {
  ...
  /**
   * Narration ids whose on-demand regeneration already failed in this
   * session. Without this, auto-play chaining through a whole course would
   * re-request a hopeless TTS provider once per line, per replay.
   */
  private readonly narrationRegenFailures = new Set<string>();
      ...
      case 'speech': {
        ...
        this.audioPlayer
          ...
          .then((audioStarted) => {
            ...
            // The document references narration this browser has no bytes for
            // (generated on another device — narration is stored locally until
            // the pool conversion lands). Regenerate it on demand under the
            // SAME id, so the bytes land in this browser's store and every
            // later replay resolves them directly.
            const settings = useSettingsStore.getState();
            const canRegenerate =
            ...
            void this.regenerateNarration(speechAction, generation).then((regenerated) => {
              if (!this.isCurrentGeneration(generation)) return;
              // On success the reused onEnded chain advances playback; on
              // failure fall back exactly as if no regeneration ran.
              if (!regenerated) useLocalFallback();
            });
          })
          .catch((err) => {
            if (!this.isCurrentGeneration(generation)) return;
  ...
  /**
   * Regenerate a missing narration clip on demand, under its existing id.
   *
   * Returns true when the regenerated bytes are playing (the already-wired
   * onEnded chain advances playback) and false when the caller should use its
   * local fallback. A generation change mid-flight reports true: the chain
   * that asked is dead, and its replacement resolves bytes on its own pass.
   */
  private async regenerateNarration(
    speechAction: SpeechAction,
    generation: number,
  ): Promise<boolean> {
    const audioId = speechAction.audioId;
    if (!audioId || this.narrationRegenFailures.has(audioId)) return false;
    try {
      const scene = this.scenes[this.sceneIndex];
      ...
      return await this.audioPlayer.play(assetId);
    } catch (error) {
      this.narrationRegenFailures.add(audioId);
      log.warn('On-demand narration regeneration failed:', error);
      return false;
    }
  }
